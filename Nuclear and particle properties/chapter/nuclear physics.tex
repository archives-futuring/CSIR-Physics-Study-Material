...
$\frac{Z^{2}}{A} \geq 44 \quad $\text{Spontaneous symmetric fission}\\
\begin{itemize}
...
\text { even: } \vec{J}=\left|j_{p}-j_{n}\right|\\
\end{array}\\
\text { Parity: } \quad &P=(-1)^{l n+l p}
\end{align*}
\textbf{Example: }$ {}_5B^{10}$
\begin{align*}
n&=5:\left(1 s_{1 / 2}\right)^{2}\left(1 p_{3} / 2\right)^{3}\quad j_{n}=\frac{3}{2}, l_{n}=1\\
P&=5:\left(1 s_{1 / 2}\right)^{2}\left(1 p_{3 / 2}\right)^{3}\quad \dot{\rho}_{p}=\frac{3}{2}, \quad l_{p}=1\\
\dot{I}_{n}+\dot{j}_{p}+l_{n}+l_{p}&=\frac{3}{2}+\frac{3}{2}+1+1=5 \text { (odd) }\\
J&=i_{N}+i_{p}=3 \\
P&=(-1)^{1+1}=\text {+ve }\\
J ^P&=3^{+}
\end{align*}
\subsubsection{Magnetic Moment }
gives schmidth curves for Proton \& neuton.
\begin{align*}
\left.\begin{array}{r}
\mu=(J+2.29) \mu_{N} \text { For } j=1+1 / 2 \text { case } \\
J+\left(1-\frac{2.29}{J+1}\right) H_{N} \text { For } j=l-1 / 2 \text { case }
\end{array}\right\}
\quad\text{ For Proton}\\\\
\left.\begin{array}{l}
H=-1.91 \mu_{N} \text { for } l+1 / 2 \text { case } \\
1.91 \frac{J}{J+1} H_{N} \text { for } l-1 / 2 \text { case }
\end{array}\right\}\quad \text{For Neutron}
\end{align*}
\end{enumerate}
\begin{exercise}
	Find magnetic moments of ${ }_{6} \mathrm{C}^{\mathrm{13}}$ and $11^{\mathrm{Na}^{23}}$ nuclei by shell model.
\end{exercise}
\begin{answer}
	\begin{align*}
...
\end{align*}
\subsubsection{Limitation of Shell Model}
\begin{itemize}
...
\subsubsection{Collective Model}
