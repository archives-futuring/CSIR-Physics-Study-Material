...
\intertext{Elements of $\hat{L}_{+}=\left\langle\ell^{\prime}, m_{l}^{\prime}\left|\hat{L}_{-}\right| \ell, m_{l}\right\rangle=\hbar \sqrt{\left(\ell+m_{l}\right)\left(\ell-m_{l}+1\right)} \delta_{\ell \ell}, \delta_{m_{l},} m_{l}-1$, will be non-zero for $l=l^{\prime}$ and $m_{l}=m_{l}^{\prime}-1$ i.e.}
...
\intertext{So $J_{+}|j m\rangle$ is eigen vector of $J_{z}$ with the eigen value $(m+1) \hbar$ i.e., $J+|j m\rangle$ can be represented as }
