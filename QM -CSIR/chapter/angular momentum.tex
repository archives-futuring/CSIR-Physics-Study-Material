...
Since,the commutator bracket $\left[ L^2,L_z\right] =0$ therefore $L^2$ and $L_z$ can be measured simultaneously accurately and both have simultaneous eigenfunctions and eigenkets.Denoting the simultaneous eigen kets $|\ell,m_\ell\rangle $ (Where l and $m_\ell$ are the orbital quantum number magnetic quantum number respectevely),the eigen value equation for $L^2$ and $L_z$ can be written as 
\begin{align*}
\hat{L}^{2}\left|\ell, m_{\ell}\right\rangle&=l(\ell+1) \hbar^{2}\left|\ell, m_{\ell}\right\rangle \\
\hat{L}_{z}\left|\ell, m_{\ell}\right\rangle&=m_{\ell} \hbar\left|\ell, m_{\ell}\right\rangle\\
...
\hat{L}^{2} Y_{\ell, m_{\ell}}(\theta, \phi)&=\ell(\ell+1) \hbar^{2} Y_{\ell, m_{\ell}}(\theta, \phi) \\
\hat{L}_{z} Y_{\ell, m_{\ell}}(\theta, \phi)&=m_{\ell} \hbar Y_{\ell, m_{\ell}}(\theta, \phi)\\
\text { where } Y_{\ell, m}(\theta, \phi) \text{is said to be}&\text {  Spherical Harmonics and defined as }\\
Y_{\ell, m_{\ell}}(\theta, \varphi)&=\varepsilon \sqrt{\left(\frac{2 \ell+1}{4 \pi}\right) \frac{\left(\ell-\left|m_{\ell}\right|\right) !}{\left(\ell+\left|m_{\ell}\right|\right) !}} P_{\ell}^{\left|m_{\ell}\right|}(\cos \theta) e^{i m_{\ell} \phi}\\
\text { where } \varepsilon&=(-1)^{m_{\ell}} \text { for } m_{\ell}>0 \text { and } \varepsilon=1 \text { for } m_{\ell} \leq 0 \text {. }
\end{align*}
...
\begin{align*}	
\hat{L}_{+}\left|\ell, m_{\ell}\right\rangle&=\sqrt{\left(\ell-m_{\ell}\right)\left(\ell+m_{\ell}+1\right)} \hbar\left|\ell, m_{\ell}+1\right\rangle \\
\hat{L}_{+} Y_{\ell, m_{\ell}}(\theta, \phi)&=\sqrt{\left(\ell-m_{\ell}\right)\left(\ell+m_{\ell}+1\right)} \hbar Y_{\ell, m_{\ell}+1}(\theta, \phi)\\
\hat{L}_{-}\left|\ell, m_{\ell}\right\rangle&=\sqrt{\left(\ell+m_{\ell}\right)\left(\ell-m_{\ell}+1\right)} \hbar\left|\ell, m_{\ell}-1\right\rangle \\
\hat{L}_{-} Y_{\left(, m_{\ell}\right.}(\theta, \phi)&=\sqrt{\left(\ell+m_{\ell}\right)\left(\ell-m_{\ell}+1\right)} \hbar Y_{\ell, m_{\ell}-1}(\theta, \phi)
\end{align*}
\subsection{ Matrix Representation of the Operators:}
	$$
	\begin{aligned}
	\text{Elements of }\hat{L}^{2}&=\left\langle\ell^{\prime}, m_{\ell}^{\prime}\left|\hat{L}^{2}\right| \ell, m_{\ell}\right\rangle=\ell(\ell+1) \hbar^{2} \delta_{\ell \ell^{\prime}} \delta_{m_{\ell} m_{\ell}^{\prime}},\text{ will be non-zero for }\ell=\ell^{\prime}\text{ and }m_{\ell}=m_{\ell^{\prime}}\text{ is}\\
	...
	0 & 1 & 0 \\
	0 & 0 & 1
	\end{array}\right](\text { for } \ell=1)\\
	\text{Elements of }\hat{L}_{z}&=\left\langle\ell^{\prime}, m_{\ell}^{\prime}\left|\hat{L}_{z}\right| \ell, m_{\ell}\right\rangle=m_{\ell} \hbar \delta_{\ell \ell^{\prime}} \delta_{m_{\ell} m_{\ell}^{\prime}}\text{, will be non-zero for } \ell=\ell^{\prime}\text{ and }m_{\ell}=m_{\ell}^{\prime}\text{ i.e.}\\
	...
	0 & 0 & -1
	\end{array}\right](\text { for } \ell=1)\\
	\text{Elements of }\hat{L}_{+}&=\left\langle\ell^{\prime}, m_{\ell}^{\prime}\left|\hat{L}_{+}\right| \ell, m_{\ell}\right\rangle=\hbar \sqrt{\left(\ell-m_{\ell}\right)\left(\ell+m_{\ell}+1\right)} \delta_{\ell \ell^{\prime}} \delta_{m_{\ell}^{\prime}, m_{\ell}+1}\text{, will be non-zero for }\ell\\&=\ell^{\prime}\text{ and } m_{\ell}=m_{\ell}^{\prime}+1\text{ i.e.}\\
	...
	\text{Elements of }\hat{L}_{-}&=\left\langle\ell^{\prime}, m_{\ell}^{\prime}\left|\hat{L}_{-}\right| \ell, m_{\ell}\right\rangle=\hbar \sqrt{\left(\ell+m_{\ell}\right)\left(\ell-m_{\ell}+1\right)} \delta_{\ell \ell^\prime}, \delta_{m_{\ell} m_{\ell}^{\prime}-1}\text{, will be non-zero for } \ell\\&=\ell^{\prime}\text{ and }m_{\ell}=m_{\ell}^{\prime}-1 \text{i.e.}\\
...
(ii) Accordinh to Schrodinger's wave theory, if the atom has an orbital angular momentum ' $\ell$ ' then the beam will split into discrete $(2 \ell+1)$ components. Since, the silver atom is in ground state $(\ell=0)$, there will be only spot on the screen.
...
Fermions: Particle having half-integer spins. Examples: quarks, electrons, protons, neutrons etc.\\
 ...
 In the Stern-Gerlach experiment, the silver atom has orbital angular momentum to be zero i.e $\ell=0$ but spin angular momentum to be $1 / 2$ i.e. $s=1 / 2$. Therefore, total angular momentum will be
 $$
 j=|\ell-s| \text { to }|\ell+s|=-\frac{1}{2}, \frac{1}{2}
 $$
 ...
 \begin{itemize}
 	...
 	\begin{align*}
 		\text{Spin-up state: }& \chi_{1 / 2}=|\uparrow\rangle=\left|\frac{1}{2}, \frac{1}{2}\right\rangle\\
 	\text{Spin-down state: }& \chi_{-1 / 2}=|\downarrow\rangle=\left|\frac{1}{2},-\frac{1}{2}\right\rangle
 \text{	such that}
 	\end{align*}
 	$$\begin{aligned}
 		&\hat{S}^{2} \chi_{1 / 2}=\frac{3}{4} \hbar^{2} \chi_{1 / 2}, \hspace{1cm} \hat{S}^{2} \chi_{-1 / 2}=\frac{3}{4} \hbar^{2} \chi_{-1 / 2} \\
 		&\hat{S}_{z} \chi_{1 / 2}=\frac{\hbar}{2} \chi_{1 / 2}, \hspace{1.3cm} \hat{S}_{z} \chi_{-1 / 2}=\frac{\hbar}{2} \chi_{-1 / 2} \\
 		&\hat{S}_{+} \chi_{1 / 2}=0,\hspace{1.9cm} \hat{S}_{+} \chi_{-1 / 2}=\hbar \chi_{1 / 2} \\
 		&\hat{S}_{-} \chi_{1 / 2}=\hbar \chi_{-1 / 2}, \hspace{1cm} \hat{S}_{-} \chi_{-1 / 2}=0
 	\end{aligned}$$
 \end{itemize}
