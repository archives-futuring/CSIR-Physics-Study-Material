...
t_{r}&=t-r / c\\
\therefore\quad \text{ Potentials become}\\
V(\mathbf{r})&=\frac{1}{4 \pi \epsilon_{0}} \int \frac{\rho\left(\mathbf{r}^{\prime}\right)}{r} d \tau^{\prime}, \quad \mathbf{A}(\mathbf{r})=\frac{\mu_{0}}{4 \pi} \int \frac{\mathbf{J}\left(\mathbf{r}^{\prime}\right)}{r} d \tau^{\prime}
\end{align*}
...
\text{This is the time-}&\text{denpendent generalzation of the Biot-savart law, to which it reduces in the static case.}
\end{align*}
...
\text{when the total field  }&\text{energy within $V$ is}\\
...
\begin{align*}
	...
	\text{$\therefore $\quad  total power }&\text{ radiated through surface is the total power radiated in space.}
\end{align*}
...
\text{For the wires in an ordinary radio }&\text{with $d=5 \mathrm{~cm}=5 \times 10^{-2} \mathrm{~m}$ and $\lambda=10^{3} \mathrm{~m}$}\\
