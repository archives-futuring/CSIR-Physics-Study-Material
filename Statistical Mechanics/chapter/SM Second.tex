...
\end{tabular}
\begin{figure}[H]
	\includegraphics[height=5.5cm,width=12.5cm]{SP-13}
\end{figure}
...
\end{tabular}\\
\begin{align*}
\text{If $E\propto p^s$ and }&\text{$d\rightarrow$ degrees of freedom,}\\
\text{then }quad g(E) \propto E^{(\frac{d}{s}-1)}
\end{align*}
...
E&\rightarrow \text{ Total energy} \\
-J \sum \sigma_{i} \sigma_{j}& \rightarrow  \text{Interaction energy}\\
-\mu_{B} H\sum \sigma_{i}&  \rightarrow \text{Interaction energy energy asosiated with magnetic field H}\\
\sigma_{i}\text{ and } \sigma_{j} &\text { are ising spins }=\pm 1\\
\textbf{Partition Function}\\
\text{Energy due to}&\text{ nearest neighbour interaction,}\\
...
\text{Energy due to }&\text{interaction with magnetic field, H}\\
...
\text{Differentiating }&\text{equation (\ref{SME-05})}\\
...
\text{The condition for the}&\text{ most probable distribution is}\\
d \Omega&=0\notag\\
d\left(\log _{e} \Omega\right)&=0\\
\text{From equation }&\text{(\ref{SME-06}) we have,}\\
-\sum_{i} \log \frac{n_{i}}{g_{i}} d n_{i}&=0\notag\\
\text { or } \quad \sum_{i} \log \frac{n_{i}}{g_{i}} d n_{i}&=0\label{SME-08}\\
\text{From equation (\ref{SME-01}) and }&\text{(\ref{SME-02}), we have}\\
\sum_{i} d n_{i}&=0
...
\text{This is the equation for the most }&\text{probable distribution according to Maxwell-Boltzmann statistics.}\\
...
\text{Then the number of molecules}&\text{ $\mathrm{dN}$ with energy between $\in$ and $\varepsilon+d \varepsilon$ can be written as,}\\
...
\text{Substituting in }&\text{equation(\ref{SME-15})}\\
...
\text{Substituting the }&\text{value of z}\\
...
\text{This is the formula}&\text{ for the energy distribution of the molecules in an ideal gas.}\\
\text{The variation $(\mathrm{dN} / \mathrm{d} \varepsilon)$}&\text{ with $\varepsilon$ is as shown in Fig. for two different temperature.}\notag
\end{align}
\begin{figure}[H]
	\centering
...
\text{Let $\mathrm{dN}$ be the number of molecules }&\text{with speeds between $v$ and $v+d v$.}\\
\frac{\mathrm{dN}}{\mathrm{dv}}&=\frac{\mathrm{dN}}{\mathrm{d} \varepsilon} \cdot \frac{\mathrm{d} \varepsilon}{\mathrm{dv}}\label{SME-19}\\
\text { Substituting equation }&\text{(\ref{SME-17}) and }(\ref{SME-18}) \text { in (\ref{SME-19}). }\\
...
\begin{answer}
	...
	\text{From equation }&\text{$(\ref{SME-24})$}\\
...
\begin{answer}
	\begin{align*}
	\text{The standard atmos}&\text{pheric pressure $=p$}\\
...
\text{But $\mathrm{dn}_{\mathrm{i}}$ is a variable quantity and is not zero. }&\text{So the coefficient of $\mathrm{dn}_{\mathrm{i}}$ must be zero.}\notag\\
...
\text{ The most probable distribution can be }&\text{found out by evaluating the maximum value of $\log _{\mathrm{e}} \mathrm{W}$.}\\
...
\text{Differentiating equation (\ref{SME-35}). }&\text{(remember $g_i$ is a constant)}\\
...
\text{For any energy }&\text{$E$, this equation can be written as,}\\
...
\text{Grand canonical}&\text{ partition function }
\end{align*}
\begin{align*}
...
\begin{enumerate}
	\item At $T=T_{B}$\\\\
	Condensation process is about to start at $T=T_{B}$It is like a tipping point and maximum number of particles are still in the exited state.
	\begin{align*}
