...
\end{align*}
$Z\rightarrow$canonical partion function\\
$\sum_{N\omega=O}$ $e^{\beta{\mu} N_{\Omega}}$
$\rightarrow$ Fugacity
\begin{align*}
\sum(\mu,\omega,T)&=Fugacity\times{Z_{canonical}}
\end{align*}
\subsection{Some Important relations connecting Grand Canonical Ensemble}
\begin{enumerate}
...
\mu=-ve\\
\text{for some bosons chemical potential is negative} \\
\text{for photons and phonons,}&\mu=0\\
\mu=0 or -ve\text{ in Bose Particles.}
\end{align*}
\subsection{Base Eienstien Codensation(v state of matter)}
...
\begin{enumerate}
	\item At $T=T_{B}$\\
	Condensation procee is about to start at $T=T_{B}$It is like a tipping point and marimum number of particles are still in hte exited state.\\
	$N=N_{0}+N_{e}$, here $N_{0}$ is negligible\\
	$N \approx N e$.\\
\textbf{Important points}\\
	$
	T_{B}=\left(\frac{N}{V}\right)^{2 / 3} \frac{h^{2}}{(2 \cdot 612)^{2 / 3} \times 2 \pi m k g_{s}}
	$\\
	$g_{S} \rightarrow$ degeneracy.\\
	$\frac{N}{V} \rightarrow$ number density $=S$\\
	$T_{B} \propto(S)^{2 / 3}$\\
...
\begin{enumerate}
\item Total number of particlen, $N^{\prime} \alpha T^{d / S}$\\
\item$V \alpha \frac{1}{T^{d/s+1}}$ \\
