...
\end{align*}
Q. A one dimensional random walder takes to steps in left and right with equal probability.  Find our the probability that the random walker starting from origine is back to origine after taking to steps?
\begin{answer}
	\begin{align*}
...
$\rightarrow$ if wall is consucting, non rigid and permeable.\\
$\bar{T}_{1}=\bar{T}_{2} ; \quad \bar{P}_{1}=\bar{P}_{2} ; \quad \bar{\mu}_{1}=\bar{\mu}_{2}$\\
\textbf{Postulate:} Equal a priori probability \\
$\rightarrow$ All microstates corresponding to a given macrostate are equally probable.\\
$\rightarrow$ \textbf{Ensemble:} collection of large number of systems that are macroscopically identical but microscopically different.\\\\
...
S&=N_{i} k \ln v_{i}+\frac{3}{2} N_{i} k \ln \left(\frac{4 \pi m_{i} E}{3 M_{i} h^{2}}\right)+\frac{3}{2} N_{i} k
\end{align*}
\textbf{Mixing of Two Gases: }\\
Volume ensity (Particle density) may be same but particles on both sides after division not same.\\
1. Mixing of two different gases will not be reversible.\\
2. Mixing of two different samples of same gas may be reversible of irreversible.\\
\textbf{Reversible Mixing (Gibbs Paradox)}\\
entropy of mixing: $\Delta S=S_{1}+S_{2}+2 N K \ln 2$\\
\textbf{Removal of Gibbs Paradox }
\begin{align*}
\Omega _{indis. }&=\frac{\Omega_{\text {dis. }}}{N !} \\
S_{indis. }&=S_{\text {dis }}-k \text { ln } N !\\
S=&N K \ln \left(\frac{V}{N}\right)+\frac{3}{2} N k \ln \left(\frac{2 \pi m K T}{h^{2}}\right)+\frac{5}{2} n K \hspace{3cm}
\end{align*}
\text { (Sackur-tetrode equation) }\\
\textbf{For Extreme Relativistic Classical Gas:}\\
...
a. For continuous energy:\\
$Q_{N}(T, V)=\left(Q_{1}(T, V)\right)^{N}$
b. For discrete energy:\\
\begin{align*}
 \theta_{N}(T, V) &=\left[\theta_{1}(T, V)\right]^{N} \rightarrow \text { distinguishable } \\ &=\frac{\left[\theta_{1}(T, V)\right]^{N}}{N !} \rightarrow \text { indistinguishable. } 
 \end{align*}
...
$\langle A\rangle=\langle E\rangle-+\langle S\rangle \Rightarrow-K_{B} T \ln z$
...
\begin{enumerate}
	...
	\begin{answer}
		...
		So the correct answer is \textbf{Option (a)}
	\end{answer}
	\item A random walder takes a step of unit length in +ve direction with probability $2/3$ and a step of unit length in -ve direction with probability $1/3$. The net displacement of random walker after $n$ steps is?
	 \begin{tasks}(2)
		\task[\textbf{a.}]$n$
		\task[\textbf{b.}]$(1 / 3)^{n}$
		\task[\textbf{c.}]$(2 / 3) n$
...
\begin{answer}
	...
	So the correct answer is \textbf{Option (b)}
\end{answer}
\item A system has two energy levels with energies $\epsilon \text{ and }2\epsilon$. The lower level is u-fold generate white the upper level is double degenerate. If there are $N$ non-interacting classical particles in the system, which is in thermodynamic equilibrium at a temperature $T$, the fraction of particles in the upper level is 
 \begin{tasks}(2)
