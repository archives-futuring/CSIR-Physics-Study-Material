\chapter{Statistical Mechanics-I}
...
\intertext{In terms of energy,}
n(E) d E&=2 g \pi V\left(\frac{2 m}{h^{2}}\right)^{3 / 2} E^{1 / 2} d E
\intertext{where '$\mathrm{g}$' is degeneracy of the particle.}
\end{align*} 
...
\intertext{Hence $P_{i}$ is probability then}
...
\end{align*}   
The letter $Z$ stands for the German word Zustandssumme, "sum over states" and is  known as \textbf{partition function} for canonical ensemble which is given by, 
\begin{equation}
...
\begin{center}
	\framebox{
		\parbox[t][1.6cm]{6cm}{
			...
			Partition Function,\quad $Z=\sum_{i} e^{-\epsilon_{i} \beta}$\\ \vspace{0.3cm}
		Probability, \quad \qquad $P_{i}=\frac{ e^{-\epsilon_{i} \beta}}{\sum_{i} e^{-\epsilon_{i} \beta}} $} }
\end{center}
...
\begin{enumerate}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[height=2cm,width=2.5cm]{diagram-20210911(13)-crop}
	\end{figure}
	\begin{tasks}(4)
...
\begin{enumerate}
	...
	\begin{answer}
		\begin{align*}
		\left\langle \epsilon_{i}\right\rangle&=\sum \epsilon_{i} P_{i}\\
